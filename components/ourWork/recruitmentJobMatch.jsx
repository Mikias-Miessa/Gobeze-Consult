  ...
  const data = [
    {
      ...
      description:
        'Founded in 1998, The Christopher Group (TCG) has become one of the nation’s largest and most respected boutique Contained Executive HR Search firms and HR Direct Staffing Solutions firms. As an HR direct staffing agency, we specialize in the sourcing, assessment.',
      button: {
        label: 'Read More',
        color: 'gray',
      },
    ...
    <div id='recruitment' className='md:mx-0 mx-4 mb-8 mt-12'>
      <div className=''>
        ...
        <h3 className='text-center text-sm text-gray-600'>
          Gobeze Consult is radically improving the hiring process for
        ...
        {data.map((item) => (
          ...
          >
            <div className={
              ...
              <p className={`text-${item.id === 1 ? 'gray-400' : 'black'}`}>
                {item.description}
              </p>
              <button
                className={`border border-${item.button.color} text-${item.button.color} font-bold text-sm px-10 py-3 my-8`}
              >
                {item.button.label}
              </button>
            </div>
